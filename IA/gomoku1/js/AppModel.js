...
var AppModel = function() {
    ...
    this.setStartData = function(a) { // Начальные установки для каждой новой игры les reglages initiaux pour chaques nouveaux jeux
        this.who = true;
        this.matrix = [];
        this.winLine = [];
    ...
    this.calculateHashMovePattern = function() { // Расчет весов потенциальных ходов по заданным шаблонам la partie strategie
        ...
        for (n in this.hashStep)
            for (m in this.hashStep[n]) // Перебор всех потенциальных ходов (*1)
            {
                ...
                this.hashStep[n][m].attackPattern = 0; // Обнуляем значение атаки по шаблону Mettre à zéro la valeur du motif d'attaque
                this.hashStep[n][m].defencePattern = 0; // Обнуляем значение защиты по шаблону La valeur du model de defense mise à zéro
                n = parseInt(n);
                m = parseInt(m);
                for (var q = 1; q <= 2; q++) // Первым проходом расчитываем веса атаки, вторым - веса защиты la premiere boucle compte le poid de l'attaque et la deuxieme celui de la defense
                    for (var j = 1; j <= 4; j++)
                    {
                        s = '';
